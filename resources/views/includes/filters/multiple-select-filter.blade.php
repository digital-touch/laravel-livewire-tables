<x-dropdown id="{{$filterName}}_filter" align="right">
    <x-slot name="trigger">
    ...
    <x-slot name="content">
        @foreach($filter->options() as $optionKey => $optionValue)
            @php($isSelected = isset($selected) && in_array($optionKey, $selected))
            <div wire:click="selectFilterValues('{{$filterName}}','{{$optionKey}}')"
