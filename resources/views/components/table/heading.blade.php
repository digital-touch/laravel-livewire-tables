...
<th scope="col"
    {{ $attributes->merge(['class' => 'px-4 py-4 '.($width ?? '')])->only('class') }}
>
    @unless ($sortable)
        <span
            class="block text-left text-xs leading-4 font-medium text-cool-gray-500 uppercase tracking-wider {{$width ?? ''}}">
            {{ $text ?? $slot }}
        </span>
    @else
        <button
